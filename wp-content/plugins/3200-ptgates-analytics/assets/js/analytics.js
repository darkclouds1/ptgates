(function ($) {
  ...
  const Analytics = {
    init: function () {
      ...
      // Auto-scroll to top when tabs are clicked
      $(document).on("click", "#e-n-tab-title", function () {
        var $target = $("#e-n-tabs-heading");
        if ($target.length) {
          var offsetTop = $target.offset().top; // 문서 기준 Y 위치
          console.log("#ptg-analytics-app offsetTop =", offsetTop);

          $("html, body").animate(
            {
              scrollTop: offsetTop - 10, // 여기 -10은 헤더/여백 보정값
            },
            400
          );
        } else {
          console.log("#e-n-tabs-heading 요소를 찾지 못했습니다.");
        }
      });
    },
    ...
    loadDashboard: function () {
      ...
      $.ajax({
        ...
        error: function (xhr, status, error) {
          ...
          self.container.html(
            '<div class="ptg-error" style="text-align:center; padding:40px;">' +
              msg +
              "</div>"
          );
        },
    ...
    renderDashboard: function (data) {
      ...
      html +=
        '<div class="ptg-chart-container"><canvas id="ptgRadarChart"></canvas></div>';
      html += "</div>";

      // Learning Velocity
      html += '<div class="ptg-section"><h3>학습 속도 (최근 7일)</h3>';
      html +=
        '<div class="ptg-chart-container"><canvas id="ptgVelocityChart"></canvas></div>';
      html += "</div>";
      ...
      html +=
        '<div class="ptg-legend">범례: (정답수 / 풀이수 / 전체문항수)</div>';
      ...
      if (data.all_subject_stats && data.all_subject_stats.length > 0) {
        ...
        for (const [parent, items] of Object.entries(grouped)) {
          ...
          const cardStyle = s
            ...
            : "";
          const headerStyle = s
            ? `background: ${s.bg} !important; color: ${s.text} !important; border-bottom-color: ${s.borderBottom} !important;`
            : "";
          ...
          items.forEach(function (item) {
            ...
            html += "<li>";
            html +=
              '<span class="ptg-subject-name" title="' +
              item.subject +
              '">' +
              item.subject +
              "</span>";
            html +=
              '<div class="ptg-progress-bar"><div class="ptg-progress-fill ' +
              colorClass +
              '" style="width:' +
              item.accuracy +
              '%"></div></div>';
            ...
            html +=
              '<div class="ptg-accuracy ' +
              colorClass +
              '-text">' +
              item.accuracy +
              "%</div>";
            html +=
              '<span class="ptg-counts">(' +
              item.correct +
              " / " +
              item.attempted +
              " / " +
        ...
        html +=
          '<p style="text-align:center; color:#999; padding:20px;">분석할 데이터가 없습니다.</p>';
      }
