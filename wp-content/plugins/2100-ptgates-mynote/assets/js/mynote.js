    ...
    const MyNote = {
        init: function () {
            ...
            if (!this.config.restUrl || !this.config.nonce) {
                console.warn('PTG MyNote: 필수 설정이 없습니다. 숏코드가 올바르게 렌더링되었는지 확인하세요.');
                return;
            }
                ...
                error: function (xhr, status, error) {
                    ...
                    if (self.container && self.container.length && listContainer.length) {
                        ...
                        if (xhr.status === 401 || xhr.status === 403) {
                            listContainer.html('<div class="ptg-error">로그인이 필요합니다.</div>');
                        } else {
                            listContainer.html('<div class="ptg-error">메모를 불러오는데 실패했습니다.</div>');
                        }
                        // 개발 환경에서만 콘솔 에러 출력
        ...
        renderList: function (data, append = false) {
            ...
            if (!items || items.length === 0) {
                listContainer.html('<div class="ptg-empty">저장된 메모가 없습니다.</div>');
                return;
            }
                ...
                <table class="ptg-memo-table">
                    <thead>
                        <tr>
                            ...
                            <th>문제ID</th>
                            <th>
                                <button class="ptg-sort-btn ${this.sortBy === 'subject' ? 'active' : ''}" data-sort="subject">
                                    과목 ${this.sortBy === 'subject' ? (this.sortOrder === 'asc' ? '↑' : '↓') : ''}
                                </button>
                            </th>
                            <th>
                                <button class="ptg-sort-btn ${this.sortBy === 'subsubject' ? 'active' : ''}" data-sort="subsubject">
                                    세부과목 ${this.sortBy === 'subsubject' ? (this.sortOrder === 'asc' ? '↑' : '↓') : ''}
                                </button>
                            </th>
                            <th>메모 내용</th>
                            <th>문제보기</th>
                            <th>
                                <button class="ptg-sort-btn ${this.sortBy === 'date' ? 'active' : ''}" data-sort="date">
                                    등록일자 ${this.sortBy === 'date' ? (this.sortOrder === 'asc' ? '↑' : '↓') : ''}
                                </button>
                            </th>
                            <th>삭제</th>
                        </tr>
            ...
            items.forEach(function (memo, index) {
                ...
                html += '<td style="text-align: center;">' + (questionId || '-') + '</td>';
                html += '<td style="text-align: center;">' + escapeHtml(mainSubject) + '</td>';
                html += '<td style="text-align: center;">' + escapeHtml(subSubject) + '</td>';
                html += '<td class="ptg-memo-content-cell" style="text-align: left;">' + escapeHtml(memoContent) + '</td>';
                html += '<td style="text-align: center;"><button class="ptg-btn ptg-btn-primary ptg-view-question" data-question-id="' + questionId + '" data-memo-content="' + escapeHtml(memoContent) + '">문제보기</button></td>';
                html += '<td style="text-align: center;">' + updatedAt + '</td>';
                html += '<td style="text-align: center;"><button class="ptg-btn ptg-btn-danger ptg-memo-delete" data-id="' + memo.id + '">삭제</button></td>';
                html += '</tr>';
            }.bind(this));
            ...
            if (totalPages > 1) {
                ...
                if (this.currentPage > 1) {
                    html += '<button class="ptg-page-btn" data-page="' + (this.currentPage - 1) + '">이전</button>';
                }
                html += '<span class="ptg-page-info">' + this.currentPage + ' / ' + totalPages + '</span>';
                if (this.currentPage < totalPages) {
                    html += '<button class="ptg-page-btn" data-page="' + (this.currentPage + 1) + '">다음</button>';
                }
                html += '</div>';
            }
            ...
            // 페이지네이션 이벤트
            this.container.off('click', '.ptg-page-btn').on('click', '.ptg-page-btn', function () {
        ...
        renderCardList: function (data, append) {
            ...
            const cardsHtml = items.map((memo) => {
                const mainSubject = memo.main_subject || '-';
                const subSubject = memo.sub_subject || '-';
                const memoContent = memo.content || '';
                const updatedAt = memo.updated_at ? memo.updated_at.split(' ')[0] : '-';
                ...
                return `
                    <article class="ptg-memo-card" data-memo-id="${memo.id || ''}">
                        <header class="ptg-memo-card-header">
                            <div class="ptg-memo-card-hgroup">
                                <span class="ptg-memo-card-subject">${escapeHtml(mainSubject)}</span>
                                <span class="ptg-memo-card-divider">|</span>
                                <span class="ptg-memo-card-subsubject">${escapeHtml(subSubject)}</span>
                            </div>
                            <span class="ptg-memo-card-id">${memoIdLabel}</span>
                        </header>
                        <div class="ptg-memo-card-body">
                            <p class="ptg-memo-card-content">${escapeHtml(memoContent)}</p>
                        </div>
                        ...
                        <footer class="ptg-memo-card-footer">
                            ...
                            <div class="ptg-card-actions">
                                ...
                                <button class="ptg-btn ptg-btn-primary ptg-view-question" data-question-id="${questionId}" data-memo-content="${escapeHtml(memoContent)}">문제보기</button>
                                <button class="ptg-btn ptg-btn-danger ptg-memo-delete" data-id="${memo.id}">삭제</button>
                            </div>
                        </footer>
                    </article>
                `;
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.question) {
                        self.renderQuestionDetail(response.question, response.memo || memoContent);
                    } else {
        ...
        renderQuestionDetail: function (question, memoContent) {
            ...
            if (typeof window.PTGQuestionViewer === 'undefined') {
                console.error('PTGQuestionViewer is not loaded. Please ensure question-viewer.js is loaded.');
            ...
            // 공통 컴포넌트로 문제 카드 렌더링
            const questionCardHtml = window.PTGQuestionViewer.renderQuestionCard(lesson, 1, {
                showToolbar: true,
                showMemo: false,
            ...
            // 대시보드 URL 가져오기
            const dashboardUrl = (window.ptgMyNote && window.ptgMyNote.dashboardUrl) || '#';
            ...
            const $lessonItem = container.find('.ptg-lesson-item[data-lesson-id="' + lesson.id + '"]');
        ...
        setupToolbarHandlers: function ($item, questionId) {
            ...
            $item.find('.ptg-contextual-action-btn').off('click').on('click', function (e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            if (typeof window.PTGStudyToolbar !== 'undefined' && window.PTGStudyToolbar.handleToolbarAction) {
                $item.find('.ptg-toolbar-btn').off('click').on('click', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
        ...
        setupStudyEventHandlers: function () {
            ...
            this.container.off('click', '.toggle-answer').on('click', '.toggle-answer', function (e) {
                e.preventDefault();
                e.stopPropagation();
                $(this).closest('.ptg-lesson-answer-area').find('.answer-content').slideToggle();
            });

            this.container.off('click', '.toggle-answer-img').on('click', '.toggle-answer-img', function (e) {
                e.preventDefault();
                e.stopPropagation();
        ...
        setupInfiniteScroll: function () {
            ...
            this.infiniteScrollHandler = function () {
                if (self.currentView !== 'list' || self.isLoadingMore || !self.hasMore) {
                    return;
                }

                const scrollTop = $(window).scrollTop();
                const windowHeight = $(window).height();
                const containerBottom = self.container.offset().top + self.container.outerHeight();

                if (scrollTop + windowHeight + 200 >= containerBottom) {
        ...
        showInfiniteLoader: function () {
            ...
            if (!listContainer.find('.ptg-infinite-loader').length) {
                listContainer.append('<div class="ptg-infinite-loader">메모를 불러오는 중...</div>');
            }
        },
        ...
        showEndMessage: function () {
            ...
            const listContainer = this.container.find('.ptg-memo-list');
            listContainer.append('<div class="ptg-infinite-end">마지막 메모까지 확인했습니다.</div>');
        ...
        maybeTriggerImmediateLoad: function () {
            ...
            const containerBottom = this.container.offset().top + this.container.outerHeight();
        ...
        enterMemoEditMode: function ($card) {
            if (!$card || !$card.length || $card.hasClass('is-editing')) {
                const $existingField = $card ? $card.find('.ptg-memo-edit-field') : null;
            ...
            const textarea = $('<textarea class="ptg-memo-edit-field" rows="4"></textarea>');
            ...
            this.showCardStatus($card, '포커스를 다른 곳으로 이동하면 자동으로 저장됩니다.', 'info');
        },
        ...
        saveMemoContent: function (memoId, content, $card) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    const updatedContent = (response && typeof response.content !== 'undefined') ? response.content : (content || '');
                    const updatedAt = response && response.updated_at ? response.updated_at.split(' ')[0] : null;
        ...
        activateToolbarMemo: function ($lessonItem, questionId) {
            ...
            const $toolbarBtn = $lessonItem.find('.ptg-contextual-action-btn').first();
