    ...
    const Bookmarks = {
        ...
        bindEvents: function() {
            ...
            this.$container.on('click', '.ptg-toggle-options', function(e) {
                ...
                $btn.text($options.hasClass('is-visible') ? '선택지 숨기기' : '선택지 보기');
            });
            ...
            this.$container.on('click', '.ptg-toggle-explanation', function(e) {
                ...
                $btn.toggleClass('is-active');
                $btn.text($explanation.hasClass('is-visible') ? '문제풀이 숨기기' : '문제풀이 보기');
            });
        ...
        loadBookmarks: function(append) {
            ...
            if (!append) {
                ...
                this.$container.html('<div class="ptg-bookmarks-loading">북마크 목록을 불러오는 중...</div>');
            }
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    if (xhr.status === 404) {
                        errorMessage = '북마크 API를 찾을 수 없습니다. 플러그인이 활성화되어 있는지 확인해주세요.';
                    ...
                    self.$container.html('<div class="ptg-bookmarks-empty">' + errorMessage + '</div>');
                    self.isLoading = false;
                }
            });
        },
        ...
        renderBookmarkCard: function(item) {
            ...
            const explanationHtml = this.renderExplanation(item.answer, item.explanation);
            ...
            if (item.subject || item.subsubject) {
                ...
                if (subject && subsubject) {
                    subjectInfo = `<span class="ptg-bookmark-subject">${this.escapeHtml(subject)} / ${this.escapeHtml(subsubject)}</span>`;
                } else if (subject) {
                    subjectInfo = `<span class="ptg-bookmark-subject">${this.escapeHtml(subject)}</span>`;
                } else if (subsubject) {
                    subjectInfo = `<span class="ptg-bookmark-subject">${this.escapeHtml(subsubject)}</span>`;
                }
            }
                ...
                <div class="ptg-bookmark-card">
                    <div class="ptg-bookmark-card-header">
                        <div class="ptg-bookmark-meta">
                            <span class="ptg-bookmark-date">${this.escapeHtml(bookmarkedDate)}</span>
                            ${subjectInfo}
                            <span class="ptg-bookmark-id">${this.escapeHtml(item.question_id_display)}</span>
                        </div>
                        <button class="ptg-bookmark-toggle ptg-toggle-options">선택지 보기</button>
                    </div>
                    <div class="ptg-bookmark-question-text"><span class="ptg-bookmark-number">${this.cardCounter}.</span> ${this.escapeHtml(item.question_text)}</div>
        ...
        renderOptions: function(options) {
            ...
            return options.map(function(option, index) {
                return `<div class="ptg-bookmark-option">${self.escapeHtml(option)}</div>`;
            }).join('');
        },
        ...
        renderExplanation: function(answer, explanation) {
            let html = '';
            if (answer) {
                html += `<div class="ptg-bookmark-answer">정답: ${this.escapeHtml(answer)}</div>`;
            }
            if (explanation) {
                html += `<div class="ptg-bookmark-explanation-content">${this.escapeHtml(explanation)}</div>`;
            }
            if (!html) {
                html = '<div class="ptg-bookmark-explanation-content">해설 정보가 없습니다.</div>';
            }
            return html;
        },
        ...
        startQuiz: function() {
            ...
            const quizUrl = (window.ptg_bookmarks_vars && window.ptg_bookmarks_vars.quiz_url) || '/ptg_quiz/';
        ...
        showMapPopup: function() {
            ...
            if (typeof window.PTGTips === 'undefined' || typeof window.PTGTips.show !== 'function') {
                console.warn('[PTG Bookmarks] 공통 팝업 유틸리티가 아직 로드되지 않았습니다. 잠시 후 다시 시도해주세요.');
                // 잠시 후 다시 시도 (최대 3초)
                let retryCount = 0;
                const maxRetries = 30;
                const self = this;
