...
- `class-subjects.php`: 교시/과목/세부과목 정적 정의 (플랫폼 코어로 이동 완료)
...
### 5. 문서화 ✅
- `readme.txt`: 플러그인 설명
- `docs/02-database-schema.md`: 통합 DB 스키마 문서 (ptgates_schema.txt 기준)
...
│       └── admin.css               # 관리자 스타일 ✅
├── docs/                           # 문서 디렉토리 ✅
│   ├── 01-architecture.md         # 프로젝트 설계서 ✅
│   ├── 02-database-schema.md      # 통합 DB 스키마 ✅
│   ├── 03-api-guide.md            # API 가이드 ✅
│   ├── 04-subject-rules.md        # 문항 선택/순서 규칙 ✅
│   ├── 05-performance.md          # 성능 최적화 ✅
│   ├── 06-development-status.md   # 개발 진행 상황 ✅
│   ├── 07-references/             # 참고 문서 ✅
│   └── ptgates_schema.txt         # 실제 DB 스키마 덤프 ✅
...
### 5. 트리거 자동 업데이트
- `study_count`/`quiz_count` 변경 시 `last_study_date`/`last_quiz_date` 자동 업데이트
- PHP 코드에서 명시적 업데이트 불필요
...
**우선순위 1:**
- ✅ 9000-ptgates-exam-questions (기출문제 참조용, Admin 전용)
