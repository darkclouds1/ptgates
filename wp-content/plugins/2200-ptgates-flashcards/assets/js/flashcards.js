    ...
    const PTGFlashApp = {
        ...
        bindEvents: function () {
            ...
            $('#ptg-sets-grid').on('click', '.ptg-btn-delete-set', function(e) {
                ...
                let msg = '정말 삭제하시겠습니까?';
                if (isDefault) msg = '이 세트의 모든 카드를 비우시겠습니까? (세트는 유지됩니다)';
        ...
        loadSets: function () {
            ...
            $.ajax({
                ...
                error: function () {
                    $('#ptg-sets-grid').html('<div class="ptg-error">세트 목록을 불러오지 못했습니다.</div>');
                }
            });
        },
        ...
        SESSION_STRUCTURE: (typeof ptgFlash !== 'undefined' && ptgFlash.subjectMap) ? ptgFlash.subjectMap : {},
        ...
        populateSubjects: function(session) {
            ...
            if (session && this.SESSION_STRUCTURE[session]) {
                ...
                Object.keys(this.SESSION_STRUCTURE).forEach(sessKey => {
                    const sessSubjects = Object.keys(this.SESSION_STRUCTURE[sessKey].subjects);
        ...
        populateSubSubjects: function(session, subject) {
            ...
            const getSubsFromSubject = (sessKey, subjKey) => {
                if (this.SESSION_STRUCTURE[sessKey] && 
                    this.SESSION_STRUCTURE[sessKey].subjects[subjKey]) {
                    return Object.keys(this.SESSION_STRUCTURE[sessKey].subjects[subjKey].subs);
                }
                return [];
            };
        ...
        updateRecommendedLimit: function() {
            ...
            if (total !== null) {
                ...
                if (!exists) {
                    limitSelect.append($('<option>').val(total).text(`${total}문제`).attr('data-auto-added', '1'));
                    // Sort options
            ...
            $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    if (xhr.status == 409) {
                        ...
                        const msg = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : (xhr.statusText || '오류가 발생했습니다.');
                        self.showToast(msg);
                    }
                }
            });
        ...
        renderSets: function (sets) {
            ...
            if (!sets || sets.length === 0) {
                container.html('<div class="ptg-empty">생성된 암기카드 세트가 없습니다.</div>');
                return;
            }
            ...
            sets.forEach(set => {
                ...
                } else if (displayName.includes('[랜덤]')) { // Legacy check
                    isRandom = true;
                }
                ...
                // Delete Button
                let deleteBtn = `<button class="ptg-btn-delete-set" data-id="${set.set_id}" data-default="${isDefault}" title="${isDefault ? '비우기' : '삭제'}">${isDefault ? '🧹' : '🗑️'}</button>`;
                
                const html = `
                    <div class="ptg-set-card ${isRandom ? 'is-random' : ''}" data-id="${set.set_id}">
        ...
        renderCard: function () {
            ...
            const frontHtml = `<span style="font-weight:bold; margin-right:5px;">${qNum}.</span>` + this.formatContent(card.front_custom);
            ...
            if (card.exam_course || card.subject) {
                ...
                if (parts.length > 0) {
                    subjectInfo = `<div style="margin-top:15px; font-size:0.85em; color:#888; text-align:right; border-top:1px solid #eee; padding-top:5px;">${parts.join(' > ')}</div>`;
                }
            }

            $('#ptg-card-front-content').html(frontHtml);
            $('#ptg-card-back-content').html(this.formatContent(card.back_custom) + subjectInfo);
        ...
        flipCard: function () {
            ...
            if (this.state.cardHeights) {
                const newHeight = this.state.isFlipped ? this.state.cardHeights.back : this.state.cardHeights.front;
        ...
        checkUsageLimit: function() {
            ...
            const limit = (this.config.limits && this.config.limits[grade]) ? this.config.limits[grade] : 0;
            ...
            if (usage >= limit) {
                if (confirm(`일일 학습 제한(${limit}개)을 초과했습니다.\n멤버십을 업그레이드 하시겠습니까?`)) {
