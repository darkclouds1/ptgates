...
(function () {
  ...
  function ensureFlashcardButton() {
    ...
    if (bookmarkBtn && reviewBtn && notesBtn && drawingBtn) {
      ...
      // 표시 보장
      const finalFlashcardBtn =
        toolbarIcons.querySelector(".ptg-btn-flashcard");
  ...
  async function toggleBookmark() {
    ...
    try {
      await window.PTGPlatform.patch(
        `ptg-quiz/v1/questions/${window.PTGQuiz?.QuizState.questionId}/state`,
        {
  ...
  async function showFlashcardModal() {
    ...
    try {
      ...
      const cardsResponse = await window.PTGPlatform.get(
        "ptg-flash/v1/cards",
        params
      );
    ...
    if (!hasDbData) {
      ...
      if (window.PTGQuiz?.QuizState.questionData) {
        ...
        const questionIdPrefix =
          "(id-" + window.PTGQuiz?.QuizState.questionId + ") ";
        if (window.PTGQuiz?.QuizState.questionData.question_text) {
          frontText =
            questionIdPrefix +
            window.PTGQuiz?.QuizState.questionData.question_text.trim();
        } else if (window.PTGQuiz?.QuizState.questionData.content) {
          frontText =
            questionIdPrefix +
            window.PTGQuiz?.QuizState.questionData.content.trim();
        }

        // 선택지 추가
        if (
          window.PTGQuiz?.QuizState.questionData.options &&
          Array.isArray(window.PTGQuiz?.QuizState.questionData.options) &&
          window.PTGQuiz?.QuizState.questionData.options.length > 0
        ) {
          window.PTGQuiz?.QuizState.questionData.options.forEach(
            (option, index) => {
        ...
        if (window.PTGQuiz?.QuizState.questionData.explanation) {
          ...
          backText += htmlToTextForFlashcard(
            window.PTGQuiz?.QuizState.questionData.explanation
          );
        }
      } else {
  ...
  async function saveFlashcard() {
    ...
    try {
      ...
      if (window.PTGQuiz?.QuizState?.questionData?.category?.subject) {
        payload.subject =
          window.PTGQuiz.QuizState.questionData.category.subject;
      ...
      const response = await window.PTGPlatform.post(
        "ptg-flash/v1/cards",
        payload
      );
  ...
  function setupToolbarEvents() {
    ...
    // 툴바 전체에 이벤트 위임으로 클릭 이벤트 추가 (모든 버튼 클릭 시 헤더로 스크롤) - 사용자 요청으로 제거
    // const toolbar = document.querySelector(".ptg-quiz-toolbar");
    // if (toolbar) {
    ...
    if (btnBookmark) {
      ...
      if (currentQuizState && currentQuizState.questionId) {
        btnBookmark.setAttribute(
          "data-question-id",
          currentQuizState.questionId
        );
      }
    ...
    if (notesTextarea) {
      ...
      const adjustHeight = () => {
        ...
        if (notesTextarea.value.trim().length > 0) {
          notesTextarea.style.height =
            notesTextarea.scrollHeight + lineHeight * 2 + "px";
        } else {
          // 내용이 없으면 CSS min-height (2줄) 적용됨
          notesTextarea.style.height = "";
        }
      };
      ...
      // 초기화 및 포커스 시 높이 조절
      notesTextarea.addEventListener("focus", adjustHeight);

      // 초기 로드 시 내용이 있으면 조절 (약간의 지연 후)
