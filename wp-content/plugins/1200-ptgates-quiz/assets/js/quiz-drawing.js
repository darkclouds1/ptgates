...
(function () {
  ...
  if (QuizState) {
    ...
    QuizState.autoAlignEnabled =
      QuizState.autoAlignEnabled !== undefined
        ? QuizState.autoAlignEnabled
        : true;
  ...
  function loadPenSettings() {
    ...
    try {
      ...
      // 자동 보정 설정 불러오기
      const savedAutoAlign = localStorage.getItem("ptg-quiz-pen-auto-align");
      if (savedAutoAlign !== null) {
        QuizState.autoAlignEnabled = savedAutoAlign === "true";
      } else {
        QuizState.autoAlignEnabled = true; // 기본값
      }
    } catch (e) {
  ...
  function savePenSettings() {
    ...
    try {
      ...
      localStorage.setItem(
        "ptg-quiz-pen-auto-align",
        String(QuizState.autoAlignEnabled)
      );
    } catch (e) {
  ...
  function initializePenMenu() {
    ...
    const colorBtn = document.querySelector(
      `.ptg-pen-color-btn[data-color="${savedColor}"]`
    );
    if (colorBtn) {
      colorBtn.classList.add("active");
    } else {
      // 저장된 색상이 없으면 기본 색상 선택
      const redColorBtn = document.querySelector(
        '.ptg-pen-color-btn[data-color="rgb(255, 0, 0)"]'
      );
      if (redColorBtn) {
    ...
    // 자동 보정 체크박스 이벤트 설정 및 저장된 값 복원
    const autoAlignCheckbox = document.getElementById("ptg-pen-auto-mode");
    if (autoAlignCheckbox) {
      autoAlignCheckbox.checked = QuizState.autoAlignEnabled;

      autoAlignCheckbox.addEventListener("change", function (e) {
        QuizState.autoAlignEnabled = e.target.checked;
        savePenSettings();
      });
    }
    ...
    if (quizCard) {
      ...
      quizCard.addEventListener("touchstart", closeMenuOnCardInteraction, {
        passive: true,
      });
    }
  }
  ...
  async function toggleDrawing(force = null) {
    ...
    if (shouldEnable) {
      // 1. 해설 강제 표시 (사용자 요청: 해설 크기에 맞춰 드로잉)
      const explanation = document.getElementById("ptg-quiz-explanation");
      const isExplanationVisible =
        explanation &&
        explanation.style.display !== "none" &&
        explanation.classList.contains("visible"); // quiz.js에서 visible 클래스 사용 여부 확인 필요하지만, display 체크가 핵심

      // 해설이 보이지 않으면 정답 확인 실행 (비동기로 실행하여 드로잉 UI 지연 방지)
      if (!isExplanationVisible || explanation.innerText.trim().length < 10) {
        if (window.PTGQuiz && window.PTGQuiz.checkAnswer) {
          window.PTGQuiz.checkAnswer();
          // DOM 렌더링 대기 없이 바로 진행 (MutationObserver가 크기 조절 처리)
        }
      }

      // 2. 메모 패널 닫기 (사용자 요청)
      if (window.PTGQuizToolbar && window.PTGQuizToolbar.toggleNotesPanel) {
        // 이미 열려있는지 확인 후 닫기 (불필요한 함수 호출/스크롤 방지)
      ...
      // 툴바로 스크롤 (사용자 요청)
      if (window.PTGQuizToolbar && window.PTGQuizToolbar.scrollToToolbar) {
        setTimeout(() => {
          window.PTGQuizToolbar.scrollToToolbar();
        }, 100);
      }

      // 스크롤 잠금 비활성화 (사용자 요청으로 제거)
      // body, html overflow hidden 처리 제거됨
  ...
  function initDrawingCanvas() {
    ...
    // 변경사항 플래그 초기화
    QuizState.isDrawingDirty = false;
  ...
  function handleMouseMove(e) {
    ...
    if (QuizState.currentStrokeId !== null) {
      const currentStroke = QuizState.strokes.find(
        (s) => s.id === QuizState.currentStrokeId
      );
      if (currentStroke) {
  ...
  function handleMouseUp(e) {
    ...
    if (
      !isBlackPen &&
      QuizState.autoAlignEnabled &&
      QuizState.drawingPoints.length >= 3 &&
      QuizState.drawingTool === "pen"
    ) {
      // 기존 타이머 취소
  ...
  function saveHistoryState() {
    ...
    const imageData = QuizState.canvasContext.getImageData(
      0,
      0,
      canvas.width,
      canvas.height
  ...
  async function loadDrawingFromServer() {
    ...
    try {
      ...
      const response = await window.PTGPlatform.get(
        `ptg-quiz/v1/questions/${QuizState.questionId}/drawings`,
        {
      ...
      if (
        ...
        if (drawing.format === "json" && drawing.data) {
          try {
            ...
            if (
              imageDataObj.width === canvasWidth &&
              imageDataObj.height === canvasHeight
            ) {
              ...
              QuizState.canvasContext.clearRect(
                0,
                0,
                canvasWidth,
                canvasHeight
              );
              QuizState.canvasContext.drawImage(
                tempCanvas,
                0,
                0,
                canvasWidth,
              ...
              const currentImageData = QuizState.canvasContext.getImageData(
                0,
                0,
                canvasWidth,
                canvasHeight
  ...
  async function saveDrawingToServer() {
    ...
    // 변경사항이 없으면 저장하지 않음 (이전/다음 문제 이동 시 성능 최적화)
    if (!QuizState.isDrawingDirty) {
      // console.log("[PTG Quiz] 드로잉 변경사항 없음, 저장 건너뜀");
      return;
    }
    ...
    try {
      ...
      // 저장 성공 여부와 관계없이 시도했으면 플래그 해제 (계속 재시도 방지)
      QuizState.isDrawingDirty = false;
    }
  }
  ...
  function setPenColor(color) {
    ...
    if (isBlack) {
      // 검은색 펜 선택 시: 자동 정렬 비활성화, 두께 3px, 불투명도 90%로 설정
      ...
      if (!wasBlack) {
        try {
          localStorage.setItem(
            "ptg-quiz-pen-width-backup",
            String(QuizState.penWidth)
          );
          localStorage.setItem(
            "ptg-quiz-pen-alpha-backup",
            String(QuizState.penAlpha)
          );
        } catch (e) {
          // localStorage 사용 불가 시 무시
        }
      ...
      // 검은색 선택 시에는 색상만 저장 (두께/불투명도는 저장하지 않음)
      try {
        localStorage.setItem("ptg-quiz-pen-color", color);
        // penWidth, penAlpha는 저장하지 않음 (검은색 전용 설정이므로)
      } catch (e) {
        // localStorage 사용 불가 시 무시
      }
    } else {
  ...
  async function clearDrawing() {
    ...
    // 빈 캔버스 상태만 히스토리에 저장
    const emptyImageData = QuizState.canvasContext.getImageData(
      0,
      0,
      canvas.width,
      canvas.height
  ...
  function drawAlignedLine(lineFit) {
    ...
    if (
      QuizState.currentStrokeStartIndex >= 0 &&
  ...
  function finishStroke() {
    ...
    if (QuizState.drawingHistory.length > QuizState.drawingHistoryIndex + 1) {
      QuizState.drawingHistory = QuizState.drawingHistory.slice(
        0,
        QuizState.drawingHistoryIndex + 1
      );
    }
  ...
  function deleteStroke(strokeId) {
    ...
    for (let i = 0; i < QuizState.strokes.length; i++) {
      ...
      if (stroke.points && stroke.points.length >= 2) {
        // 선 시작 전 상태 저장
        const beforeImageData = QuizState.canvasContext.getImageData(
          0,
          0,
          canvas.width,
          canvas.height
        ...
        // 선 완료 후 상태 저장
        const afterImageData = QuizState.canvasContext.getImageData(
          0,
          0,
          canvas.width,
          canvas.height
    ...
    if (QuizState.drawingHistory.length > 50) {
      ...
      QuizState.strokes.forEach((stroke) => {
        if (stroke.startHistoryIndex >= 0) {
          stroke.startHistoryIndex = Math.max(
            0,
            stroke.startHistoryIndex - removeCount
          );
        }
        if (stroke.endHistoryIndex >= 0) {
          stroke.endHistoryIndex = Math.max(
            0,
            stroke.endHistoryIndex - removeCount
          );
        }
      });
    }
