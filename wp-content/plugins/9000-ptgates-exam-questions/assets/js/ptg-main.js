...
(function () {
  ...
  async function loadQuestions(filters = {}) {
    ...
    if (filters.full_session)
      params.append("full_session", filters.full_session ? "1" : "0");
  ...
  async function startQuiz() {
    ...
    const subject =
      document.getElementById("ptgates-filter-subject")?.value || "";
    ...
    if (typeof PTGTimer !== "undefined") {
      ...
      if (isSession1 || isSession2) {
        const timeMode =
          document.querySelector('input[name="ptgates-time-mode"]:checked')
            ?.value || "unlimited";
        if (timeMode === "limited") {
          const minutes = parseInt(
            document.getElementById("ptgates-time-minutes")?.value || "0",
            10
          );
          if (minutes > 0) {
  ...
  function submitAnswer() {
    ...
    // 다음 버튼 표시
    document
      .getElementById("ptgates-submit-btn")
      ?.style.setProperty("display", "none");
    document
      .getElementById("ptgates-next-btn")
  ...
  function giveUpQuiz() {
    ...
    if (
      currentQuestion &&
      QuizState.answers.length === QuizState.currentIndex
    ) {
      // 현재 문제에 대한 답안이 없으면, 빈 답안으로 처리
      const userAnswer =
        typeof PTGUI !== "undefined" ? PTGUI.getSelectedAnswer() : "";
      if (userAnswer) {
  ...
  function init() {
    ...
    if (shortcodeLimit) {
      ...
      if (limitSelect) {
        const option = limitSelect.querySelector(
          `option[value="${shortcodeLimit}"]`
        );
        if (option) {
    ...
    const timeMinutesSuffix = document.getElementById(
      "ptgates-time-minutes-suffix"
    );
    if (limitSelect) {
      limitSelect.addEventListener("change", (e) => {
        ...
        if (sessionTimeRow) {
          sessionTimeRow.style.display =
            isSession1 || isSession2 ? "flex" : "none";
        }
        // 기본 시간값 및 라디오 초기화
        if (isSession1 || isSession2) {
          ...
          const unlimitedRadio = document.querySelector(
            'input[name="ptgates-time-mode"][value="unlimited"]'
          );
          const limitedRadio = document.querySelector(
            'input[name="ptgates-time-mode"][value="limited"]'
          );
          if (limitedRadio) limitedRadio.checked = true;
          if (timeMinutesInput) timeMinutesInput.style.display = "inline-block";
          if (timeMinutesSuffix)
            timeMinutesSuffix.style.display = "inline-block";
        } else {
          // 숨김 및 초기화
          const unlimitedRadio = document.querySelector(
            'input[name="ptgates-time-mode"][value="unlimited"]'
    ...
    if (timeModeRadios && timeModeRadios.length) {
      timeModeRadios.forEach((radio) => {
        radio.addEventListener("change", (e) => {
          ...
          if (mode === "limited") {
            if (timeMinutesInput)
              timeMinutesInput.style.display = "inline-block";
            if (timeMinutesSuffix)
              timeMinutesSuffix.style.display = "inline-block";
          } else {
            if (timeMinutesInput) timeMinutesInput.style.display = "none";
