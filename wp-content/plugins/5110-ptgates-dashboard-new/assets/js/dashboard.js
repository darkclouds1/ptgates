(function ($) {
  ...
  const Dashboard = {
    ...
    init: function () {
      this.$container = $("#ptg-dashboard-new-app");
    ...
    loadTabContent: function (tabId) {
      const $contentArea = $("#ptg-tab-content-container");
      let $tabPane = $("#ptg-tab-pane-" + tabId);

      // 1. Hide all existing panes and placeholder
      $contentArea.children().hide();

      // 2. If pane exists, show it and return (DOM Cache)
      if ($tabPane.length > 0) {
        $tabPane.show();
        return;
      }
      ...
      // 3. Create new pane
      $tabPane = $(
        `<div id="ptg-tab-pane-${tabId}" class="ptg-tab-pane"></div>`
      );
      $contentArea.append($tabPane);

      // 4. Show Loading inside the new pane
      $tabPane.html(`
            <div class="ptg-tab-loading">
                <div class="ptg-tab-spinner"></div>
                <span>데이터를 불러오는 중...</span>
            </div>
        `);
      $tabPane.show();

      const restUrl = window.ptg_dashboard_new_vars
        ? window.ptg_dashboard_new_vars.rest_url
        : "/wp-json/ptg-dash-new/v1/";
      const nonce = window.ptg_dashboard_new_vars
        ? window.ptg_dashboard_new_vars.nonce
        : "";

      $.ajax({
        url: restUrl + "tab-content",
        method: "GET",
        data: { tab: tabId },
        ...
        success: function (response) {
          if (response && response.html) {
            ...
            $tabPane.html(
              '<div class="ptg-tab-placeholder">불러올 내용이 없습니다.</div>'
            );
          }
        },
        error: function () {
          $tabPane.html(
            '<div class="ptg-tab-placeholder" style="color:#ef4444;">데이터를 불러오지 못했습니다.</div>'
          );
        },
      });
    },
    ...
    bindEvents: function () {
      ...
      // --- Tab Handling ---
      this.$container.on("click", ".ptg-tab-btn", function (e) {
        e.preventDefault();
        const $btn = $(this);
        const tabId = $btn.data("tab");

        // UI Update
        self.$container.find(".ptg-tab-btn").removeClass("is-active");
      ...
      this.$container.on(
        ...
        function (e) {
          ...
          if (subjectName) {
            ...
            let studyBaseUrl =
              (window.ptg_dashboard_new_vars &&
                window.ptg_dashboard_new_vars.study_url) ||
              "";
      ...
      // 사용자 데이터 초기화 버튼
      // const self = this; // Already declared at top of bindEvents
    ...
    openPricingGuide: function () {
      ...
      $.get(
        "/wp-content/plugins/5110-ptgates-dashboard-new/assets/html/pricing-guide.html",
        function (data) {
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response && response.success) {
            // window.ptg_dashboard_new_vars 업데이트 (즉시 반영)
            if (window.ptg_dashboard_new_vars) {
              window.ptg_dashboard_new_vars.study_count = 0;
              window.ptg_dashboard_new_vars.quiz_count = 0;
              window.ptg_dashboard_new_vars.flashcard_count = 0; // 암기카드도 삭제됨
            }
            ...
            if ($usageItems.length >= 2) {
              ...
              if ($studyValue.length) {
                const studyLimit =
                  window.ptg_dashboard_new_vars?.study_limit >= 999999
                    ? "무제한"
                    : (
                        window.ptg_dashboard_new_vars?.study_limit || 0
                      ).toLocaleString();
              ...
              if ($quizValue.length) {
                const quizLimit =
                  window.ptg_dashboard_new_vars?.quiz_limit >= 999999
                    ? "무제한"
                    : (
                        window.ptg_dashboard_new_vars?.quiz_limit || 0
                      ).toLocaleString();
              ...
              if ($flashcardValue.length) {
                const flashcardLimit =
                  window.ptg_dashboard_new_vars?.flashcard_limit >= 999999
                    ? "무제한"
                    : (
                        window.ptg_dashboard_new_vars?.flashcard_limit || 0
                      ).toLocaleString();
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data && typeof data === "object") {
            // window.ptg_dashboard_new_vars 업데이트 (최신 데이터 반영)
            if (window.ptg_dashboard_new_vars) {
              if (data.flashcard && data.flashcard.total !== undefined) {
                window.ptg_dashboard_new_vars.flashcard_count =
                  data.flashcard.total || 0;
              }
              if (data.study_progress !== undefined) {
                window.ptg_dashboard_new_vars.study_count =
                  data.study_progress || 0;
              }
              if (data.quiz_progress !== undefined) {
                window.ptg_dashboard_new_vars.quiz_count =
                  data.quiz_progress || 0;
              }
              if (data.limits) {
                window.ptg_dashboard_new_vars.study_limit =
                  data.limits.study || 0;
                window.ptg_dashboard_new_vars.quiz_limit =
                  data.limits.quiz || 0;
                window.ptg_dashboard_new_vars.flashcard_limit =
                  data.limits.flashcard || 0;
              }
            }
            ...
            if (window.ptg_dashboard_new_vars) {
              ...
              if ($usageItems.length >= 3) {
                ...
                if ($flashcardValue.length) {
                  ...
                  $flashcardValue.html(
                    `${(
                      window.ptg_dashboard_new_vars?.flashcard_count || 0
    ...
    render: function (data) {
      ...
      const welcomeHtml = `
        ...
        window.ptg_dashboard_new_vars?.study_limit === -1 ||
        window.ptg_dashboard_new_vars?.study_limit >= 999999
          ? "무제한"
          : (window.ptg_dashboard_new_vars?.study_limit || 0).toLocaleString()
      }
                                </div>
                            </div>
                            <div class="ptg-usage-item">
        ...
        window.ptg_dashboard_new_vars?.quiz_limit === -1 ||
        window.ptg_dashboard_new_vars?.quiz_limit >= 999999
          ? "무제한"
          : (window.ptg_dashboard_new_vars?.quiz_limit || 0).toLocaleString()
      }
                                </div>
                            </div>
                            <div class="ptg-usage-item">
        ...
        window.ptg_dashboard_new_vars?.flashcard_limit === -1 ||
        window.ptg_dashboard_new_vars?.flashcard_limit >= 999999
          ? "무제한"
          : (
              window.ptg_dashboard_new_vars?.flashcard_limit || 0
                    ...
                    <section class="ptg-mb-section">
                        ...
                        <div class="ptg-account-links">
                            <a href="${
                              window.ptg_dashboard_new_vars?.account_url || "#"
      ...
      // 5. Tabs Section (Replaces static Learning Records)
      const tabsHtml = `
            <!-- Lazy Loading Tabs -->
            <div class="ptg-dashboard-tabs">
                <div class="ptg-tab-nav">
                    <button type="button" class="ptg-tab-btn" data-tab="stats">📊 학습 통계</button>
                    <button type="button" class="ptg-tab-btn" data-tab="subject">📝 과목별</button>
                    <button type="button" class="ptg-tab-btn" data-tab="date">🗓 일자별</button>
                </div>
                <div id="ptg-tab-content-container" class="ptg-tab-content-area">
                    <div class="ptg-tab-placeholder">
            ...
            <style>
                .ptg-dashboard-tabs { margin-top: 30px; margin-bottom: 40px; }
                .ptg-tab-nav { display: flex; justify-content: center; gap: 10px; margin-bottom: 20px; flex-wrap: wrap; }
                .ptg-tab-btn { padding: 8px 20px; background: #fff; border: 1px solid #e2e8f0; border-radius: 999px; font-size: 14px; font-weight: 600; color: #64748b; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; gap: 6px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
                .ptg-tab-btn:hover { background: #f8fafc; color: #334155; }
                .ptg-tab-btn.is-active { background: #3b82f6; color: #ffffff; border-color: #3b82f6; box-shadow: 0 4px 6px -1px rgba(59, 130, 246, 0.5); }
                .ptg-tab-content-area { min-height: 200px; }
                .ptg-tab-placeholder { padding: 40px; text-align: center; color: #94a3b8; border: 2px dashed #e2e8f0; border-radius: 12px; background: #f8fafc; }
                .ptg-tab-loading { padding: 40px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #64748b; }
                .ptg-tab-spinner { width: 30px; height: 30px; border: 3px solid #e2e8f0; border-top-color: #3b82f6; border-radius: 50%; animation: ptg-spin 0.8s linear infinite; margin-bottom: 12px; }
                @keyframes ptg-spin { to { transform: rotate(360deg); } }
            </style>
      `;

      // Combine all sections (Row 1: Welcome, Row 2: Cards Grid, Row 3: Tabs)
      this.$container.html(welcomeHtml + cardsHtml + tabsHtml);
